...
static struct inode*
namex(char *path, int nameiparent, char *name, struct inode *basediri)
{
  ...
  if(*path == '/') {
    ...
    basediri = basediri ? basediri : proc->cwd;
    ip = idup(basediri);
  }
...
struct inode*
nameibase(char *path, struct inode *base)
{
  char name[DIRSIZ];
  return namex(path, 0, name, base);
}
